...
class NetResolverCreator {
 public:
    ...
    void createNetResolverForNet(Module<NS>& module, Net<NS>& net) {
        ...
        Vid procName = IRDBImpl<NS>::get().createNetResolver(drive - io, io, net.getDataType());
        Process<NS>& process = IRDBImpl<NS>::get().getProcess(procName);
...
void ElabDB<NS>::createVertex(const ElabAnnotator<NS>& a) {
    const Module<NS>& root = IRDBImpl<NS>::get().getRoot();
...
template class ElabDB<IR_DEFAULT>;
}
