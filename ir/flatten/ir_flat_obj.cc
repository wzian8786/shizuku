...
Vid FMInst<NS>::getName() const {
    uint32_t instId = IRDBImpl<NS>::get().getMInstID(_dfs);
...
std::string FMInst<NS>::getPath(unsigned char delimiter) const {
    const IRDBImpl<NS>& db = IRDBImpl<NS>::get();
    uint32_t dfs = _dfs;
    std::vector<Vid> names;
    while (dfs) {
        names.emplace_back(FMInst<NS>(dfs).getName());
        uint32_t instId = db.getMInstID(dfs);
        dfs -= db.getDFSOffset(instId);
    }
    std::stringstream ss;
...
FMInst<NS> FMInst<NS>::getParent() const {
    const IRDBImpl<NS>& db = IRDBImpl<NS>::get();
    uint32_t instId = db.getMInstID(_dfs);
    uint64_t dfs = _dfs - db.getDFSOffset(instId);
...
size_t FMInst<NS>::Pool::getMaxSize() {
    const IRDBImpl<NS>& db = IRDBImpl<NS>::get();
    return db.getTotalNumInst(db.getRoot().getID());
}
...
template class FMInst<IR_DEFAULT>;
template class FPInst<IR_DEFAULT>;
template class FDPort<IR_DEFAULT>;
template class FRPort<IR_DEFAULT>;
